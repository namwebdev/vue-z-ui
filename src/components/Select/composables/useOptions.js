...
export default function useOptions(props, context, dep) {
  ...
  const eo = computed(() => {
    ...
    eo = eo.map((val) => {
      return typeof val === "object"
        ? val
        : { [valueProp.value]: val, [trackBy.value]: val, [label.value]: val };
    });
  ...
  const fo = computed(() => {
    ...
    if (search.value && filterResults.value) {
      fo = fo.filter((option) => {
        return (
          normalize(option[trackBy.value]).indexOf(normalize(search.value)) !==
          -1
        );
      });
    }
  ...
  const hasSelected = computed(() => {
    ...
    switch (mode.value) {
      case "single":
        val = !isNullish(iv.value[valueProp.value]);
        break;
      case "multiple":
        break;
      case "tags":
        val = !isNullish(iv.value) && iv.value.length > 0;
        break;
      default:
        val = "";
        break;
  ...
  const deselect = (option) => {
    ...
    switch (mode.value) {
      ...
      case "multiple":
        update(
          iv.value.filter(
            (val) => val[valueProp.value] != option[valueProp.value]
          )
        );
        break;
    }
  ...
  const handleOptionClick = (option) => {
    ...
    switch (mode.value) {
      ...
      case "tags":
        ...
        if (
          getOption(option[valueProp.value]) === undefined &&
          createTag.value
        ) {
  ...
  const getOption = (val) => {
    return eo.value[
      eo.value.map((o) => String(o[valueProp.value])).indexOf(String(val))
    ];
  };
  ...
  const getOptionByTrackBy = (val) => {
    return eo.value
      .map((o) => normalize(o[trackBy.value]))
      .indexOf(normalize(val));
  };
  ...
  const makeInternal = (val) => {
    ...
    return mode.value === "single"
      ? getOption(val) || {}
      : val.filter((v) => !!getOption(v)).map((v) => getOption(v));
  };
        ...
        setTimeout(() => {
          ...
          options.value(search.value).then((response) => {
            if (query == search.value) {
              ro.value = response;
              pointer.value =
                fo.value.filter((o) => o.disabled !== true)[0] || null;
              resolving.value = false;
            }
          });
        }, delay.value);
    ...
    (newValue) => {
      ...
      switch (mode.value) {
        case "single":
          if (
            object.value
              ? newValue[valueProp.value] != iv.value[valueProp.value]
              : newValue != iv.value[valueProp.value]
          ) {
            iv.value = makeInternal(newValue);
          }
          break;
        ...
        case "tags":
          if (
            !arraysEqual(
              object.value ? newValue.map((o) => o[valueProp.value]) : newValue,
              iv.value.map((o) => o[valueProp.value])
            )
          ) {
            iv.value = makeInternal(newValue);
          }
